...
test('추천 빌드: 빈 스킬 슬롯 채움·더 나은 장비 교체(비파괴)', async () => {
  ...
  const baseline = computePower(u); // 최적화 전 전투력(무기 미장착)
  ...
  // optimize는 슬롯별 그리디 휴리스틱이라 특정 무기를 보장하진 않는다(무작위 부옵션에
  //   두 무기 전투력이 근접하면 선택이 갈림). 보유 무기 장착 + 빌드 향상만 결정론적으로 단언.
  assert.ok(u.gear.weapon && (u.gear.weapon.uid === a.uid || u.gear.weapon.uid === b.uid), '보유 무기 장착');
  assert.ok(computePower(u) > baseline, '빌드 전투력 향상');
